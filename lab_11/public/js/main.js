...
function createAppeal() {
    ...
    if (isOnline()) {
        ...
        xhr.open("POST", '/sendAppeal', false);
        ...
        xhr.send("appeal=" + text1+"&date="+date+"&time="+time+"&user=Fanatic");
...
function add_news() {
    ...
    if (bool) {
        ...
        if (isOnline()) {
            ...
            var xhr = new XMLHttpRequest();
            xhr.open("POST", '/sendNews', true);
            xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
            xhr.onreadystatechange = function() {
                if (this.readyState === XMLHttpRequest.DONE && this.status === 200) {
                }
            }
            xhr.send("title=" + title+"&text="+news+"&img="+img_file.name);
        } else {
